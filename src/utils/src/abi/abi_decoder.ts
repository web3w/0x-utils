...
// import { AbiEncoder } from "../../index"

import { create,Method  } from './abi_encoder/evm_data_type_factory'
...
export class AbiDecoder {
  ...
  public tryToDecodeLogOrNoop<ArgsType extends DecodedLogArgs>(log: LogEntry): LogWithDecodedArgs<ArgsType> | RawLog {
    ...
    const decodedNonIndexedData = create(_.filter(event.inputs, { indexed: false })).decodeAsArray(
      log.data
    )
  ...
  private _addEventABI(eventAbi: EventAbi, ethersInterface: Interface): void {
    ...
    const topic = ethersInterface.getEventTopic(eventAbi.name) //.getTopic()
  ...
  private _addMethodABI(methodAbi: MethodAbi, contractName?: string): void {
    const abiEncoder = new Method(methodAbi)
